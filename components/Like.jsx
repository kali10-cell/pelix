...
export default function Like({ idPeli }) {
  // Importar los estados de Zustand
  ...
  // Estado local para likear la peli

  // Función de like o no like
  ...
  return (
    <button
      onClick={() => togglePeliFavorita(idPeli)}
      aria-label={favoritas.includes(idPeli) ? "Quitar de favoritos" : "Añadir a favoritos"}
    ...
    >
      ...
      >
        <path
          ...
          className={`stroke-red-800 transition-all duration-200 hover:scale-110 ${favoritas.includes(idPeli) ? "fill-red-500" : "fill-red-500/20"}`}
        />
      </svg>
    </button>
  )
